...
export type propsTypes = {
  navigation: StackNavigationProp<RootStackParamList, "Works">;
};

/** MainFunction */
let Works: React.FC<propsTypes> = (props) => {
  /** @kind: Props */
  ...
  let checker = async () => {
    ...
    let value = await whereAllValues<Works>("Works", {
      key: "cv_id",
      value: Cv.cv.ID,
    });
    if (value) {
  ...
  let store = async () => {
    ...
    let newValue = await createValue<Works>(
  ...
  let remove = async (ID: string) => {
    let values = await removeValue<Works>("Works", {
      key: "ID",
      value: ID,
    });
    if (values) {
